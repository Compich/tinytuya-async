...
class Cloud(object):
    def __init__(self, apiRegion=None, apiKey=None, apiSecret=None, apiDeviceID=None, new_sign_algorithm=True, initial_token=None, **extrakw):
        ...
        """
        # Class Variables
        self.CONFIGFILE = 'tinytuya_async.json'
    ...
    async def _tuyaplatform(self, uri, action='GET', post=None, ver='v1.0', recursive=False, query=None, content_type=None):
        """
        ...
        if action == 'GET':
            ...
            log.debug(
                "GET: URL=%s HEADERS=%s response code=%d text=%s token=%s", url, headers, response.status, await response.text(), self.token
            )
        else:
            log.debug(
            ...
            log.debug(
                "POST RESPONSE: code=%d text=%s token=%s", response.status, await response.text(), self.token
            )

        # Check to see if token is expired
        if "token invalid" in await response.text():
            ...
            if not self.token:
                ...
                return await self._tuyaplatform(uri, action, post, ver, True)
        ...
        try:
            ...
            self.error = error_json(
                ERR_CLOUDKEY,
                "Cloud _tuyaplatform() invalid response: %r" % await response.text(),
            )
            return self.error
        # Check to see if token is expired
    ...
    async def _gettoken(self):
        ...
        self.token = None
        response_dict = await self._tuyaplatform('token?grant_type=1')
    ...
    async def _getuid(self, deviceid=None):
        ...
        response_dict = await self._tuyaplatform(uri)
    ...
    async def cloudrequest(self, url, action=None, post=None, query=None):
        """
        ...
        if action is None:
            action = 'POST' if post else 'GET'
        return await self._tuyaplatform(url, action=action, post=post, ver=None, query=query)
    ...
    async def _get_all_devices( self, uid=None, device_ids=None ):
        fetches = 0
        our_result = { 'result': [] }
        last_row_key = None
        has_more = True
        ...
        while has_more:
            result = await self.cloudrequest( uri, query=query )
            fetches += 1
            has_more = False
    ...
    async def getdevices(self, verbose=False, oldlist=[], include_map=False):
        """
        ...
        if self.apiDeviceID and self.use_old_device_list:
            ...
            for uid in uid_list:
                ...
                json_run = await self._tuyaplatform(uri)
                # merge the dicts
                for k in json_run:
            ...
            json_data = await self._get_all_devices()
            users = {}
            ...
            if users:
                ...
                for uid in users.keys():
                    json_data2 = await self._get_all_devices( uid=uid )
    ...
    async def _getdevice(self, param='status', deviceid=None):
        if not self.token:
            return self.error
        if not deviceid:
            return error_json(
    ...
    async def getstatus(self, deviceid=None):
        """
        Get the status of the device.
        """
        return await self._getdevice('status', deviceid)

    async def getfunctions(self, deviceid=None):
        """
        Get the functions of the device.
        """
        return await self._getdevice('functions', deviceid)

    async def getproperties(self, deviceid=None):
        """
        Get the properties of the device.
        """
        return await self._getdevice('specification', deviceid)

    async def getdps(self, deviceid=None):
        """
        ...
        response_dict = await self._tuyaplatform(uri, ver='v1.1')
    ...
    async def sendcommand(self, deviceid=None, commands=None, uri='iot-03/devices/'):
        """
        ...
        response_dict = await self._tuyaplatform(uri,action='POST',post=commands)
    ...
    async def getconnectstatus(self, deviceid=None):
        """
        ...
        uri = 'devices/%s' % (deviceid)
        response_dict = await self._tuyaplatform(uri, ver='v1.0')

        if not response_dict['success']:
            log.debug("Error from Tuya Cloud: %r", response_dict['msg'])
        return(response_dict["result"]["online"])

    async def getdevicelog(self, deviceid=None, start=None, end=None, evtype=None, size=0, max_fetches=50, start_row_key=None, params=None):
        """
        ...
        ret = await self.cloudrequest( '/v1.0/devices/%s/logs' % deviceid, query=params)
        max_fetches -= 1
        fetches = 1
        ...
        if ret and 'result' in ret:
            ...
            while (
                ...
                res = await self.cloudrequest( '/v1.0/devices/%s/logs' % deviceid, query=params)
    ...
    async def getmapping( self, productid, deviceid=None ):
        ...
        if deviceid:
            ...
            result = await self.getdps(deviceid)
