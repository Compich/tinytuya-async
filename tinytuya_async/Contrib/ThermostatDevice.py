...
"""
 ...
 This module attempts to provide everything needed so there is no need to import the base tinytuya_async module
 ...
 Additional Classes
    ...
    ThermostatSchedule class:
        ...
        !! WARNING !! The thermostat does NOT send the current schedule when you request the status, it only sends it when it has changed.  So, you
            must either a) set the entire schedule or b) change it in the app or on the thermostat itself while tinytuya_async is running.  Changes to a
...
class ThermostatSensorList(object):
    ...
    I.e.
        ...
        payload = d.generate_payload(tinytuya_async.CONTROL, data)
        d.send(payload)
    """
