...
async def main():
    ...
    res = await c.sendcommand(device_id, cloud_cmd)
    ...
    remote_list = await c.cloudrequest('/v2.0/infrareds/' + device_id + '/remotes')
    ...
    remote_key_list = await c.cloudrequest('/v2.0/infrareds/%s/remotes/%s/keys' % (device_id, remote_id))
    ...
    res = await c.cloudrequest('/v2.0/infrareds/%s/remotes/%s/command' % (device_id, remote_id), post=post_data)
    print(json.dumps(res, indent=2))

    # The actual value sent by the above key can be found by checking the device logs
    print('Device logs:')
    logs = await c.getdevicelog(device_id, evtype='5', size=3, max_fetches=1)
