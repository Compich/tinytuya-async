...
import ttcorefunc as tinytuya_async
import socket
import select
import time
import json
...
msg = tinytuya_async.TuyaMessage(1, tinytuya_async.UDP_NEW, 0, bcast_data, 0, True, tinytuya_async.PREFIX_6699_VALUE, True)
bcast_data = tinytuya_async.pack_message(msg,hmac_key=tinytuya_async.udpkey)
...
while True:
    ...
    for sock in r:
        ...
        #print(data.hex())
        m = tinytuya_async.unpack_message(data,hmac_key=tmp_key, no_retcode=True)
        #print('payload len:', len(m.payload), 'tuya message:', m)
        print('decoded message:', m)

        if m.cmd == tinytuya_async.SESS_KEY_NEG_START:
            tmp_key = real_key
            payload = m.payload
            remote_nonce = payload
            miv = remote_nonce[:12]
            hmac_check = hmac.new(real_key, remote_nonce, sha256).digest()
            msg = tinytuya_async.TuyaMessage(seqno, tinytuya_async.SESS_KEY_NEG_RESP, 0, local_nonce+hmac_check, 0, True, tinytuya_async.PREFIX_6699_VALUE, True)
            seqno += 1
            data = tinytuya_async.pack_message(msg, hmac_key=tmp_key)
            print( 'session neg start:', msg )
            client.sendall( data )
        elif m.cmd == tinytuya_async.SESS_KEY_NEG_FINISH:
            ...
            cipher = tinytuya_async.AESCipher( real_key )
        ...
        elif m.cmd == tinytuya_async.DP_QUERY_NEW:
            ...
            msg = tinytuya_async.TuyaMessage(seqno, 16, 0, json.dumps(resp).encode('ascii'), 0, True, tinytuya_async.PREFIX_6699_VALUE, True)
            seqno += 1
            data = tinytuya_async.pack_message(msg, hmac_key=tmp_key)
            client.sendall( data )
        else:
            print('unhandled command', m.cmd)
            msg = tinytuya_async.TuyaMessage(seqno, 16, 0, b'json obj data unvalid', 0, True, tinytuya_async.PREFIX_6699_VALUE, True)
            seqno += 1
            data = tinytuya_async.pack_message(msg, hmac_key=tmp_key)
            client.sendall( data )
